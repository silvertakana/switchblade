...
async function handleChat(req, res, cfg, bodyText) {
  ...
  for (const attempt of pool) {
    ...
    // One attempt record per attempt, with the final outcome (success status or
    // error). Records are appended in attempt order; a retry field (if a future
    // parallel thread adds retry metadata to candidates) is tolerated as absent.
    const attemptRec = {
